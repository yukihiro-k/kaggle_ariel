...
if __name__ == "__main__":
    ...
    if Config.CALIBRATE_WITH_FGS:
        ...
        if abs(s_corr - 1.0) > 1e-3:
            if Config.CALIBRATE_RECOMPUTE:
                ...
                # 今回は提出 μ には直接影響しない（次回実行時のための係数確認用）
                print(f"[Calibrate] s_corr={s_corr:.5f}（再学習OFFのため適用は保留）")

    # AIRS: 固定 or 自動スムージング＆ブレンド
    if Config.AIRS_ADAPTIVE:
        sigma_air_vec = estimate_sigma_air(preprocessed_data, Config)  # 自動法の重みに使用
    ...
    # sigma_air_vec は上で作成済み（AIRS_ADAPTIVE=False のときはここで作る）
    if not Config.AIRS_ADAPTIVE:
