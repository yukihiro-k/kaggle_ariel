...
class Config:
    ...
    # AIRS smoothing & blend (既定:固定法 / 下の ADAPTIVE=True で自動法に切替)
    ...
    # === 自動適応スムージングのハイパラ ===
    AIRS_ADAPTIVE = True
    AIRS_SMOOTH_GRID = [9, 11, 13, 15]     # 窓の候補（奇数に正規化）
    AIRS_BLEND_GRID  = [0.5, 0.65, 0.8]    # 生:平滑の比率（s = β*sm + (1-β)*raw）
    AIRS_ADAPT_ALPHA = 0.55                # 0:忠実度重視, 1:なめらかさ重視の重み
    AIRS_WINSOR_PCT  = (0.2, 99.8)         # 各惑星行でのウィンザー化（外れ値抑制）

    # === 追加: Tikhonov 1次差分スムージング（軽量な三重対角解法） ===
    AIRS_TIKHONOV = False                  # True にすると λ 探索で自動スムージング
    AIRS_TIKH_LAMS = np.logspace(-4, 0.7, 8)  # 候補 λ

    # === 追加: OOT のみで AIRS 重みを作る ===
    PREPROC_AIRS_USE_OOT_WEIGHTS = True

    # FGSと1D深さスケールの軽い自己キャリブ
    CALIBRATE_WITH_FGS = True          # 有効化フラグ
    CALIBRATE_RECOMPUTE = False        # Trueなら再特徴量→MLP再推論まで行う（重い）

    # === 追加: MC Dropout Test-Time Ensemble ===
    MC_DROPOUT_PASSES = 64
    MC_GAMMA_AIRS = 0.6   # MC 分散を σ に反映する規模（AIRS）
    MC_GAMMA_FGS  = 0.6   # 同上（FGS）
...
class SignalProcessor:
    ...
    def _preprocess_calibrated_signal(self, calibrated_signal, sensor):
        ...
        # === 改良: AIRS の列重みを OOT のみの分散から作成 ===
        if sensor == "AIRS-CH0":
            var = np.nanvar(binned, axis=0, ddof=1)  # fallback 用
            if self.cfg.PREPROC_AIRS_USE_OOT_WEIGHTS:
                white = np.nanmean(binned, axis=1)
                white_s = savgol_filter(white, 21, 2)
                p1, p2 = _phase_detector_signal(white_s, self.cfg)
                d = self.cfg.MODEL_OPTIMIZATION_DELTA
                p1 = max(d, p1); p2 = min(len(white) - d - 1, p2)
                oot_idx = np.r_[0:max(p1-d,0), min(p2+d, len(white)) : len(white)]
                if oot_idx.size >= 5:
                    var = np.nanvar(binned[oot_idx], axis=0, ddof=1)
...
class TransitModel:
    ...
    def predict(self, single_preprocessed_signal):
        ...
        # 安全な SavGol（長さが短いケースも考慮）
        L = len(sig)
        win = min(23, L if L % 2 == 1 else L - 1)
        if win >= 3:
            sig = savgol_filter(sig, win, 2)
...
class ResNetMLP(nn.Module):
    def __init__(self, input_dim=3, hidden_dim=32, output_dim=1, num_blocks=80, dropout_rate=0.2):
        super().__init__()
...
class ResNetMLP2(nn.Module):
    def __init__(self, input_dim=3, hidden_dim=128, output_dim=282, num_blocks=80, dropout_rate=0.3):
        super().__init__()
        self.input_layer = nn.Linear(input_dim, hidden_dim)
...
# =========================
# AIRS 平滑（安全版＆自動適応＋オプション: Tikhonov）
# =========================
...
def _roughness(arr):
    d1 = np.diff(arr, n=1, axis=1)
    return np.nanmean(d1*d1, axis=1)
...
def _fidelity(smooth, raw, sig=None):
    ...
    if sig is not None:
        if sig.ndim == 1:
            ...
            w = 1.0 / np.maximum(sig, 1e-12)
    else:
        w = 1.0
    res = (smooth - raw)
    return np.nanmean((res*res)*w, axis=1)

def airs_adaptive_smooth(pred2, cfg, sigma_air_vec=None, sigma_air_matrix=None):
    raw = pred2.copy()
    ...
    for w in cfg.AIRS_SMOOTH_GRID:
        ...
        for beta in cfg.AIRS_BLEND_GRID:
            ...
            sig = sigma_air_matrix if sigma_air_matrix is not None else sigma_air_vec
            J = cfg.AIRS_ADAPT_ALPHA*_roughness(s) + (1-cfg.AIRS_ADAPT_ALPHA)*_fidelity(s, raw, sig)
            take = J < best_J
            if np.any(take):
                best[take] = s[take]
                best_J[take] = J[take]
...
def airs_tikhonov_smooth(raw, lam, w=None):
    """min_s ||s - raw||_W^2 + lam ||D1 s||^2  (D1: 1次差分)
    w: (M,) 対角重み（1/σ^2 推奨）
    """
    M = raw.shape[1]
    if M < 3:
    ...
    if w is None:
        w = np.ones(M)
    # 三重対角 A = W + lam * D1^T D1
    # D1^T D1 の対角は [1, 2, 2, ..., 2, 1],  1次オフは [-1, ..., -1]
    main = w + lam * np.r_[1, np.full(M-2, 2.0), 1]
    off  = -lam * np.ones(M-1)
    ...
    for i in range(raw.shape[0]):
        d = w * raw[i]
        out[i] = _solve_tridiagonal(a, b, c, d)
    return out
...
def calibrate_scale_with_fgs(one_d_depth_scaled, fgs_mu):
    # s*one_d ≈ fgs_mu を最小二乗で推定（5–95%分位で外れ値を粗除外）
...
# =========================
# 追加: MC Dropout 推論ユーティリティ
# =========================
def _enable_mc_dropout(model):
    """Dropout は train、BatchNorm は eval、他は eval のままにする。"""
...
@torch.no_grad()
def mc_dropout_predict(model, X, passes, out_dim):
    model.eval()
    preds = []
    ...
    for _ in range(passes):
        y = model(X).cpu().numpy()
        preds.append(y)
    preds = np.stack(preds, axis=0)  # (T, N, D)
...
if __name__ == "__main__":
    ...
    # ---- 追加: MC Dropout TTA ----
    m = Config.MC_DROPOUT_PASSES
    with torch.no_grad():
        pred1_mean, pred1_var = mc_dropout_predict(model_fgs, X_tensor, passes=m, out_dim=1)
        pred2_mean, pred2_var = mc_dropout_predict(model_airs, X_tensor, passes=m, out_dim=282)

    predictions1 = pred1_mean / 10000.0      # (N,1)
    predictions2 = pred2_mean / 10000.0      # (N,282)

    # --- 軽い自己キャリブ（任意：FGS μと1D深さの整合）---
    if Config.CALIBRATE_WITH_FGS:
        s_corr = calibrate_scale_with_fgs(predictions, predictions1.reshape(-1))
        if abs(s_corr - 1.0) > 1e-3:
            if Config.CALIBRATE_RECOMPUTE:
                # 再特徴量→再推論（計算コストあり）
                predictions *= s_corr
                ...
                with torch.inference_mode():
                    pred1_mean, pred1_var = mc_dropout_predict(model_fgs, X_tensor, passes=m, out_dim=1)
                    pred2_mean, pred2_var = mc_dropout_predict(model_airs, X_tensor, passes=m, out_dim=282)
                predictions1 = pred1_mean / 10000.0
                predictions2 = pred2_mean / 10000.0
            else:
                print(f"[Calibrate] s_corr={s_corr:.5f}（再学習OFFのため適用は保留）")

    # σ 推定（提出にも使う）
    sigma_fgs_vec = estimate_sigma_fgs(preprocessed_data, Config)
    sigma_air_vec = estimate_sigma_air(preprocessed_data, Config)

    # ---- MC 分散を σ へ反映（過信しない程度に） ----
    # var は (μの単位)^2 / 1e8 なので √var を μ と同スケールに揃えてから加算
    mc_std_fgs  = np.sqrt(np.nan_to_num(pred1_var.reshape(-1), nan=0.0)) / 10000.0
    mc_std_airs = np.sqrt(np.nan_to_num(pred2_var, nan=0.0)) / 10000.0

    sigma_fgs_vec = np.sqrt(sigma_fgs_vec**2 + (Config.MC_GAMMA_FGS * mc_std_fgs)**2)
    # AIRS はスムージングの fidelity 重み用にも使うため、行×波長の行列を用意
    sigma_air_mat = np.sqrt((sigma_air_vec.reshape(-1,1))**2 + (Config.MC_GAMMA_AIRS * mc_std_airs)**2)

    # AIRS: 固定 or 自動スムージング＆ブレンド or Tikhonov
    if Config.AIRS_TIKHONOV:
        # λ をスコア（roughness/fidelity）の合成で選ぶ
        raw = predictions2.copy()
        best = raw.copy(); best_J = np.full(raw.shape[0], np.inf)
        for lam in Config.AIRS_TIKH_LAMS:
            w = 1.0 / np.maximum(np.nanmedian(sigma_air_mat, axis=1), 1e-12)  # 行ごとに一定重み
            s = airs_tikhonov_smooth(raw, lam=lam, w=w)
            J = Config.AIRS_ADAPT_ALPHA*_roughness(s) + (1-Config.AIRS_ADAPT_ALPHA)*_fidelity(s, raw, sigma_air_mat)
            take = J < best_J
            if np.any(take):
                best[take] = s[take]
                best_J[take] = J[take]
        predictions2 = best
    elif Config.AIRS_ADAPTIVE:
        predictions2 = airs_adaptive_smooth(predictions2, Config, sigma_air_matrix=sigma_air_mat)
    else:
...
# 変更点サマリ:
# - AIRS 列重みを OOT のみに変更（Transit 形状の影響を受けにくく）
# - MC Dropout により μ を TTA 平均、分散で不確かさを推定
# - 自動スムージングで σ（MC/光度）重みを 2D で利用、より不確か領域を強めに平滑
# - オプションで Tikhonov（1次差分）正則化スムージングを追加（デフォルト OFF）
# - 小さな安定化（SavGol 安全化、print デバッグ削除 など）